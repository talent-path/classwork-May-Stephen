...
namespace GroupByExercise
{
    class Program
    {
        ...
        {
            Dictionary<City, List<Person>> peopleByCity = new Dictionary<City, List<Person>>();
            for (int i = 0; i < allPeople.Count; i++)
            {
                if (peopleByCity.ContainsKey(allPeople[i].HomeCity))
                {
                    peopleByCity[allPeople[i].HomeCity].Add(allPeople[i]);
                }
                else
                {
                    peopleByCity.Add(allPeople[i].HomeCity, new List<Person> { allPeople[i] });
                }
